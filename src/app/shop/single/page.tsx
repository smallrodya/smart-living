...
export default function SinglePage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const singleProducts = data.products.filter(
        (product: Product) => 
          product.category === 'BEDDING' && 
          product.beddingSizes?.some(size => size.size === 'Single')
      );
      ...
      try {
        ...
        const validItems = Array.isArray(existingItems) 
          ? existingItems.filter(item => 
              ...
              !item.id.startsWith('single_')
            )
          : [];
        
        const newItems = products
          .filter((p) => newWishlist.includes(`single_${p._id}`))
          .map((item) => ({
            id: `single_${item._id}`,
            src: item.images?.[0] || '',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Discover our collection of single size bedding, perfect for individual beds and cozy spaces
              </p>
            </div>
          </div>
        </div>
                ...
                <div style={{
                  ...
                  <button
                    ...
                    style={{
                      ...
                      transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                      transform: wishlist.includes(`single_${product._id}`) ? 'scale(1.1)' : 'scale(1)',
                      backdropFilter: 'blur(4px)'
                    }}
                  >
                    <svg
                      ...
                      height="24"
                      viewBox="0 0 24 24"
                      fill={wishlist.includes(`single_${product._id}`) ? '#e53935' : 'none'}
                      stroke="#e53935"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
