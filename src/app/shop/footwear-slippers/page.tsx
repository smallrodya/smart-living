...
export default function FootwearSlippersPage() {
  ...
  const allSizes = Array.from(new Set(products.flatMap(p => p.footwearSizes?.map(s => s.size) || [])));
  const allColors = Array.from(new Set(products.flatMap(p => p.footwearColors || [])));
  ...
  const getMinPrices = (product: Product) => {
    ...
    for (const size of product.footwearSizes) {
      if (typeof size.salePrice === 'number' && size.salePrice < minSale) minSale = size.salePrice;
      if (typeof size.regularPrice === 'number' && size.regularPrice < minRegular) minRegular = size.regularPrice;
    }
  ...
  const filteredProducts = products.filter(product => {
    const matchesSize = !selectedSize || product.footwearSizes?.some(s => s.size === selectedSize);
    const matchesColor = !selectedColor || product.footwearColors?.includes(selectedColor);
    const [minPrice, maxPrice] = priceRange;
    const productPrices = product.footwearSizes?.map(s => s.salePrice) || [];
      ...
      try {
        ...
        const validItems = Array.isArray(existingItems) 
          ? existingItems.filter(item => 
              ...
              !item.id.startsWith('slippers_')
            )
          : [];
        
        const newItems = products
          .filter((p) => newWishlist.includes(`slippers_${p._id}`))
          .map((item) => ({
            id: `slippers_${item._id}`,
            src: item.images?.[0] || '',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Discover our collection of comfortable slippers
              </p>
            </div>
          </div>
        </div>
                  ...
                  >
                    <button
                      ...
                      style={{
                        ...
                        transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                        transform: wishlist.includes(`slippers_${product._id}`) ? 'scale(1.1)' : 'scale(1)',
                        backdropFilter: 'blur(4px)'
                      }}
                    >
                      <svg
                        ...
                        height="20"
                        viewBox="0 0 24 24"
                        fill={wishlist.includes(`slippers_${product._id}`) ? '#e53935' : 'none'}
                        stroke="#e53935"
                        strokeWidth="2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
