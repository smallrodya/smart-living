...
export default function CribPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const cribProducts = data.products.filter(
        (product: Product) => 
          product.category === 'BEDDING' && 
          product.beddingSizes?.some(size => size.size === 'Crib')
      );
      setProducts(cribProducts);
    } catch (error) {
      ...
      try {
        ...
        const validItems = Array.isArray(existingItems) 
          ? existingItems.filter(item => 
              ...
              !item.id.startsWith('crib_')
            )
          : [];
        
        const newItems = products
          .filter((p) => newWishlist.includes(`crib_${p._id}`))
          .map((item) => ({
            id: `crib_${item._id}`,
            src: item.images?.[0] || '',
    ...
    <>
      ...
      <main>
        ...
        <div style={{
          ...
          <Image
            src="/crib-duvet107.jpg"
            alt="Crib Size Bedding"
            fill
            style={{
              objectFit: 'cover',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Discover our adorable collection of crib size bedding, perfect for your little one's nursery
              </p>
            </div>
          </div>
        </div>
                ...
                <div style={{
                  ...
                  <button
                    ...
                    style={{
                      ...
                      transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                      transform: wishlist.includes(`crib_${product._id}`) ? 'scale(1.1)' : 'scale(1)',
                      backdropFilter: 'blur(4px)'
                    }}
                  >
                    <svg
                      ...
                      height="24"
                      viewBox="0 0 24 24"
                      fill={wishlist.includes(`crib_${product._id}`) ? '#e53935' : 'none'}
                      stroke="#e53935"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
