...
export default function PrintedPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const printedProducts = data.products.filter(
        (product: Product) => 
          product.category === 'BEDDING' && 
          product.beddingStyles?.includes('Printed')
      );
      setProducts(printedProducts);
    } catch (error) {
      ...
      try {
        ...
        const validItems = Array.isArray(existingItems) 
          ? existingItems.filter(item => 
              ...
              !item.id.startsWith('printed_')
            )
          : [];
        
        const newItems = products
          .filter((p) => newWishlist.includes(`printed_${p._id}`))
          .map((item) => ({
            id: `printed_${item._id}`,
            src: item.images?.[0] || '',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Discover our vibrant collection of printed bedding, featuring unique patterns and designs to transform your bedroom
              </p>
            </div>
          </div>
        </div>
                ...
                <div style={{
                  ...
                  <button
                    ...
                    style={{
                      ...
                      transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                      transform: wishlist.includes(`printed_${product._id}`) ? 'scale(1.1)' : 'scale(1)',
                      backdropFilter: 'blur(4px)'
                    }}
                  >
                    <svg
                      ...
                      height="24"
                      viewBox="0 0 24 24"
                      fill={wishlist.includes(`printed_${product._id}`) ? '#e53935' : 'none'}
                      stroke="#e53935"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
