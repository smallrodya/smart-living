...
export default function ClothingBathrobesPage() {
    ...
    try {
      ...
      const filteredProducts = data.products.filter((product: Product) => {
        ...
        return product.category === 'CLOTHING' && 
               product.clothingStyles?.includes('Bathrobes');
      });
  ...
  const toggleWishlist = (id: string) => {
    setWishlist(prev => {
      const prefixedId = `bathrobes_${id}`;
      ...
      try {
        ...
        const validItems = Array.isArray(existingItems) 
          ? existingItems.filter(item => 
              ...
              !item.id.startsWith('bathrobes_')
            )
          : [];
        
        const newItems = products
          .filter((p) => newWishlist.includes(`bathrobes_${p._id}`))
          .map((item) => ({
            id: `bathrobes_${item._id}`,
            src: item.images?.[0] || '',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Discover our collection of luxurious bathrobes
              </p>
            </div>
          </div>
        </div>
                ...
                <div style={{
                  ...
                  <button
                    ...
                    style={{
                      ...
                      transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                      transform: wishlist.includes(`bathrobes_${product._id}`) ? 'scale(1.1)' : 'scale(1)',
                      backdropFilter: 'blur(4px)'
                    }}
                  >
                    <svg
                      ...
                      height="24"
                      viewBox="0 0 24 24"
                      fill={wishlist.includes(`bathrobes_${product._id}`) ? '#e53935' : 'none'}
                      stroke="#e53935"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
