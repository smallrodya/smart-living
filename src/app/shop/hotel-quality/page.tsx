...
export default function HotelQualityPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const hotelQualityProducts = data.products.filter(
        (product: Product) => 
          product.category === 'BEDDING' && 
          product.beddingStyles?.includes('Hotel Quality')
      );
      setProducts(hotelQualityProducts);
    } catch (error) {
      ...
      try {
        ...
        const validItems = Array.isArray(existingItems) 
          ? existingItems.filter(item => 
              ...
              !item.id.startsWith('hotel_')
            )
          : [];
        
        const newItems = products
          .filter((p) => newWishlist.includes(`hotel_${p._id}`))
          .map((item) => ({
            id: `hotel_${item._id}`,
            src: item.images?.[0] || '',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Experience the luxury of hotel-quality bedding in your own home, featuring premium materials and exceptional craftsmanship
              </p>
            </div>
          </div>
        </div>
                ...
                <div style={{
                  ...
                  <button
                    ...
                    style={{
                      ...
                      transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                      transform: wishlist.includes(`hotel_${product._id}`) ? 'scale(1.1)' : 'scale(1)',
                      backdropFilter: 'blur(4px)'
                    }}
                  >
                    <svg
                      ...
                      height="24"
                      viewBox="0 0 24 24"
                      fill={wishlist.includes(`hotel_${product._id}`) ? '#e53935' : 'none'}
                      stroke="#e53935"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
