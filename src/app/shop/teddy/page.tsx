...
export default function TeddyPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const teddyProducts = data.products.filter(
        (product: Product) => 
          product.category === 'BEDDING' && 
          product.beddingStyles?.includes('Teddy')
      );
      ...
      try {
        ...
        const validItems = Array.isArray(existingItems) 
          ? existingItems.filter(item => 
              ...
              !item.id.startsWith('teddy_')
            )
          : [];
        
        const newItems = products
          .filter((p) => newWishlist.includes(`teddy_${p._id}`))
          .map((item) => ({
            id: `teddy_${item._id}`,
            src: item.images?.[0] || '',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Discover our cozy collection of teddy bedding, featuring soft and luxurious designs that bring warmth and comfort to your bedroom
              </p>
            </div>
          </div>
        </div>
                ...
                <div style={{
                  ...
                  <button
                    ...
                    style={{
                      ...
                      transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                      transform: wishlist.includes(`teddy_${product._id}`) ? 'scale(1.1)' : 'scale(1)',
                      backdropFilter: 'blur(4px)'
                    }}
                  >
                    <svg
                      ...
                      height="24"
                      viewBox="0 0 24 24"
                      fill={wishlist.includes(`teddy_${product._id}`) ? '#e53935' : 'none'}
                      stroke="#e53935"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
