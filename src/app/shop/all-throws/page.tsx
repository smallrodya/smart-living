...
export default function AllThrowsPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const throws = data.products.filter(
        (product: Product) => 
          product.category === 'THROWS & TOWELS' && 
          product.subcategory === 'Throws'
      );
      setProducts(throws);
    } catch (error) {
      ...
      try {
        ...
        const validItems = Array.isArray(existingItems) 
          ? existingItems.filter(item => 
              ...
              !item.id.startsWith('throw_')
            )
          : [];
        
        const newItems = products
          .filter((p) => newWishlist.includes(`throw_${p._id}`))
          .map((item) => ({
            id: `throw_${item._id}`,
            src: item.images?.[0] || '',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Discover our collection of luxurious throws for your home
              </p>
            </div>
          </div>
        </div>
                  ...
                  >
                    <button
                      ...
                      style={{
                        ...
                        transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                        transform: wishlist.includes(`throw_${product._id}`) ? 'scale(1.1)' : 'scale(1)',
                        backdropFilter: 'blur(4px)'
                      }}
                    >
                      <svg
                        ...
                        height="20"
                        viewBox="0 0 24 24"
                        fill={wishlist.includes(`throw_${product._id}`) ? '#e53935' : 'none'}
                        stroke="#e53935"
                        strokeWidth="2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
