...
export default function KingPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const kingProducts = data.products.filter(
        (product: Product) => 
          product.category === 'BEDDING' && 
          product.beddingSizes?.some(size => size.size === 'King')
      );
      ...
      try {
        ...
        const validItems = Array.isArray(existingItems) 
          ? existingItems.filter(item => 
              ...
              !item.id.startsWith('king_')
            )
          : [];
        
        const newItems = products
          .filter((p) => newWishlist.includes(`king_${p._id}`))
          .map((item) => ({
            id: `king_${item._id}`,
            src: item.images?.[0] || '',
    ...
    <>
      ...
      <main>
        ...
        <div style={{
          ...
          <Image
            src="/king-duvet107.jpg"
            alt="King Size Bedding"
            fill
            style={{
              objectFit: 'cover',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Discover our luxurious collection of king size bedding, perfect for spacious bedrooms and ultimate comfort
              </p>
            </div>
          </div>
        </div>
                ...
                <div style={{
                  ...
                  <button
                    ...
                    style={{
                      ...
                      transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                      transform: wishlist.includes(`king_${product._id}`) ? 'scale(1.1)' : 'scale(1)',
                      backdropFilter: 'blur(4px)'
                    }}
                  >
                    <svg
                      ...
                      height="24"
                      viewBox="0 0 24 24"
                      fill={wishlist.includes(`king_${product._id}`) ? '#e53935' : 'none'}
                      stroke="#e53935"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
