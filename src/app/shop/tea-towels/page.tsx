...
export default function TeaTowelsPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const teaTowels = data.products.filter(
        (product: Product) => 
          (product.category === 'THROWS & TOWELS' && 
          product.subcategory === 'Tea Towels') ||
          (product.additionalCategories && 
           product.additionalCategories.some(
             (ac: { category: string; subcategory: string }) => 
               ...
               ac.subcategory === 'Tea Towels'
           ))
      );
      setProducts(teaTowels);
    } catch (error) {
      ...
      try {
        ...
        const validItems = Array.isArray(existingItems) 
          ? existingItems.filter(item => 
              ...
              !item.id.startsWith('tea_')
            )
          : [];
        
        const newItems = products
          .filter((p) => newWishlist.includes(`tea_${p._id}`))
          .map((item) => ({
            id: `tea_${item._id}`,
            src: item.images?.[0] || '',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Discover our collection of high-quality tea towels, perfect for your kitchen
              </p>
            </div>
          </div>
        </div>
                ...
                <div style={{
                  ...
                  <button
                    ...
                    style={{
                      ...
                      transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                      transform: wishlist.includes(`tea_${product._id}`) ? 'scale(1.1)' : 'scale(1)',
                      backdropFilter: 'blur(4px)'
                    }}
                  >
                    <svg
                      ...
                      height="24"
                      viewBox="0 0 24 24"
                      fill={wishlist.includes(`tea_${product._id}`) ? '#e53935' : 'none'}
                      stroke="#e53935"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
