...
export default function PlainThrowsTowelsPage() {
    ...
    try {
      ...
      const filteredProducts = data.products.filter((product: Product) => {
        ...
        return product.category === 'THROWS & TOWELS' && 
               (product.throwsTowelsStyles?.includes('Plain') || 
                product.throwsTowelsStylePrices?.some(style => style.size === 'Plain'));
      });
      ...
      try {
        ...
        const validItems = Array.isArray(existingItems) 
          ? existingItems.filter(item => 
              ...
              !item.id.startsWith('plain_')
            )
          : [];
        
        const newItems = products
          .filter((p) => newWishlist.includes(`plain_${p._id}`))
          .map((item) => ({
            id: `plain_${item._id}`,
            src: item.images?.[0] || '',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Discover our collection of elegant plain throws and towels
              </p>
            </div>
          </div>
        </div>
                  ...
                  >
                    <button
                      ...
                      style={{
                        ...
                        transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                        transform: wishlist.includes(`plain_${product._id}`) ? 'scale(1.1)' : 'scale(1)',
                        backdropFilter: 'blur(4px)'
                      }}
                    >
                      <svg
                        ...
                        height="20"
                        viewBox="0 0 24 24"
                        fill={wishlist.includes(`plain_${product._id}`) ? '#e53935' : 'none'}
                        stroke="#e53935"
                        strokeWidth="2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
