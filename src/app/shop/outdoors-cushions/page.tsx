...
export default function OutdoorCushionsPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const outdoorCushionsProducts = data.products.filter(
        (product: Product) => product.category === 'OUTDOOR' && product.subcategory === 'Cushions'
      );
      setProducts(outdoorCushionsProducts);
    } catch (error) {
  ...
  const getMinPrices = (product: Product) => {
    ...
    return { sale: Math.min(...salePrices), regular: Math.min(...regularPrices) };
  };
  ...
  const filteredProducts = products.filter(product => {
    ...
    if (product.category === 'OUTDOOR' && product.outdoorPrice) {
      return matchesColor && matchesStyle && 
        product.outdoorPrice.salePrice >= minPrice && 
        product.outdoorPrice.salePrice <= maxPrice;
    }
    ...
    return matchesSize && matchesColor && matchesStyle && 
      ...
      try {
        ...
        const validItems = Array.isArray(existingItems) 
          ? existingItems.filter(item => 
              ...
              !item.id.startsWith('outdoor_')
            )
          : [];
        
        const newItems = products
          .filter((p) => newWishlist.includes(`outdoor_${p._id}`))
          .map((item) => ({
            id: `outdoor_${item._id}`,
            src: item.images?.[0] || '',
      ...
      <main>
        ...
        <div style={{
          ...
          <Image
            ...
            alt="Outdoor Cushions Collection"
            fill
            style={{
              objectFit: 'cover',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Discover our premium outdoor cushions for your perfect outdoor living space
              </p>
            </div>
          </div>
        </div>
                  ...
                  >
                    <button
                      ...
                      style={{
                        ...
                        transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                        transform: wishlist.includes(`outdoor_${product._id}`) ? 'scale(1.1)' : 'scale(1)',
                        backdropFilter: 'blur(4px)'
                      }}
                    >
                      <svg
                        ...
                        height="20"
                        viewBox="0 0 24 24"
                        fill={wishlist.includes(`outdoor_${product._id}`) ? '#e53935' : 'none'}
                        stroke="#e53935"
                        strokeWidth="2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
