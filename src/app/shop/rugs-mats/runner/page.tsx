...
export default function RunnerRugsMatsPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const runnerRugsMats = data.products.filter(
        (product: Product) => 
          product.category === 'RUGS & MATS' && 
          product.rugsMatsSizes?.some(size => size.size === 'Runner')
      );
      setProducts(runnerRugsMats);
    } catch (error) {
      ...
      try {
        ...
        const validItems = Array.isArray(existingItems) 
          ? existingItems.filter(item => 
              ...
              !item.id.startsWith('runner_')
            )
          : [];
        
        const newItems = products
          .filter((p) => newWishlist.includes(`runner_${p._id}`))
          .map((item) => ({
            id: `runner_${item._id}`,
            src: item.images?.[0] || '',
    ...
    <>
      ...
      <main>
        ...
        <div style={{
          ...
          <Image
            src="/runner-rugs-mats.jpg"
            alt="Runner Rugs & Mats"
            fill
            style={{
              objectFit: 'cover',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Discover our elegant collection of runner rugs and mats, perfect for hallways and narrow spaces
              </p>
            </div>
          </div>
        </div>
                  ...
                  >
                    <button
                      ...
                      style={{
                        ...
                        transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                        transform: wishlist.includes(`runner_${product._id}`) ? 'scale(1.1)' : 'scale(1)',
                        backdropFilter: 'blur(4px)'
                      }}
                    >
                      <svg
                        ...
                        height="20"
                        viewBox="0 0 24 24"
                        fill={wishlist.includes(`runner_${product._id}`) ? '#e53935' : 'none'}
                        stroke="#e53935"
                        strokeWidth="2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
