...
export default function CompleteBeddingSetPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const completeBeddingSets = data.products.filter(
        (product: Product) => 
          (product.category === 'BEDDING' && 
          product.subcategory === 'Complete Bedding Sets') ||
          (product.additionalCategories && 
           product.additionalCategories.some(
             (ac: { category: string; subcategory: string }) => 
               ac.category === 'BEDDING' && 
               ac.subcategory === 'Complete Bedding Sets'
           ))
      );
      console.log('Filtered complete bedding sets:', completeBeddingSets); // Для отладки
      setProducts(completeBeddingSets);
    } catch (error) {
      ...
      try {
        ...
        const validItems = Array.isArray(existingItems) 
          ? existingItems.filter(item => 
              ...
              !item.id.startsWith('complete_')
            )
          : [];
        
        const newItems = products
          .filter((p) => newWishlist.includes(`complete_${p._id}`))
          .map((item) => ({
            id: `complete_${item._id}`,
            src: item.images?.[0] || '',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Discover our luxurious collection of complete bedding sets, perfect for creating your dream bedroom
              </p>
            </div>
          </div>
        </div>
                ...
                <div style={{
                  ...
                  <button
                    ...
                    style={{
                      ...
                      transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                      transform: wishlist.includes(`complete_${product._id}`) ? 'scale(1.1)' : 'scale(1)',
                      backdropFilter: 'blur(4px)'
                    }}
                  >
                    <svg
                      ...
                      height="24"
                      viewBox="0 0 24 24"
                      fill={wishlist.includes(`complete_${product._id}`) ? '#e53935' : 'none'}
                      stroke="#e53935"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
