...
export default function DoublePage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const doubleProducts = data.products.filter(
        (product: Product) => 
          product.category === 'BEDDING' && 
          product.beddingSizes?.some(size => size.size === 'Double')
      );
      setProducts(doubleProducts);
    } catch (error) {
      ...
      try {
        ...
        const validItems = Array.isArray(existingItems) 
          ? existingItems.filter(item => 
              ...
              !item.id.startsWith('double_')
            )
          : [];
        
        const newItems = products
          .filter((p) => newWishlist.includes(`double_${p._id}`))
          .map((item) => ({
            id: `double_${item._id}`,
            src: item.images?.[0] || '',
    ...
    <>
      ...
      <main>
        ...
        <div style={{
          ...
          <Image
            src="/double-duvet107.jpg"
            alt="Double Size Bedding"
            fill
            style={{
              objectFit: 'cover',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Explore our collection of double size bedding, perfect for couples and spacious bedrooms
              </p>
            </div>
          </div>
        </div>
                ...
                <div style={{
                  ...
                  <button
                    ...
                    style={{
                      ...
                      transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                      transform: wishlist.includes(`double_${product._id}`) ? 'scale(1.1)' : 'scale(1)',
                      backdropFilter: 'blur(4px)'
                    }}
                  >
                    <svg
                      ...
                      height="24"
                      viewBox="0 0 24 24"
                      fill={wishlist.includes(`double_${product._id}`) ? '#e53935' : 'none'}
                      stroke="#e53935"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
