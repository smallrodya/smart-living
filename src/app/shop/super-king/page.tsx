...
export default function SuperKingPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const superKingProducts = data.products.filter(
        (product: Product) => 
          product.category === 'BEDDING' && 
          product.beddingSizes?.some(size => size.size === 'Super King')
      );
      setProducts(superKingProducts);
    } catch (error) {
  ...
  const toggleWishlist = (id: string) => {
    setWishlist(prev => {
      const prefixedId = `super_king_${id}`;
      ...
      try {
        ...
        const validItems = Array.isArray(existingItems) 
          ? existingItems.filter(item => 
              ...
              !item.id.startsWith('super_king_')
            )
          : [];
        
        const newItems = products
          .filter((p) => newWishlist.includes(`super_king_${p._id}`))
          .map((item) => ({
            id: `super_king_${item._id}`,
            src: item.images?.[0] || '',
    ...
    <>
      ...
      <main>
        ...
        <div style={{
          ...
          <Image
            src="/super-king-duvet107.jpg"
            alt="Super King Size Bedding"
            fill
            style={{
              objectFit: 'cover',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Experience ultimate luxury with our premium collection of super king size bedding, perfect for the most spacious bedrooms
              </p>
            </div>
          </div>
        </div>
                ...
                <div style={{
                  ...
                  <button
                    ...
                    style={{
                      ...
                      transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                      transform: wishlist.includes(`super_king_${product._id}`) ? 'scale(1.1)' : 'scale(1)',
                      backdropFilter: 'blur(4px)'
                    }}
                  >
                    <svg
                      ...
                      height="24"
                      viewBox="0 0 24 24"
                      fill={wishlist.includes(`super_king_${product._id}`) ? '#e53935' : 'none'}
                      stroke="#e53935"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
