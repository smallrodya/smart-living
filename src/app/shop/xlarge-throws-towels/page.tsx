...
export default function XLargeThrowsTowelsPage() {
    ...
    try {
      ...
      const filteredProducts = data.products.filter((product: Product) => {
        console.log('Проверяем товар:', {
          ...
          styles: product.throwsTowelsStyles,
          stylePrices: product.throwsTowelsStylePrices,
          subcategory: product.subcategory
        });
        
        return product.category === 'THROWS & TOWELS' && 
               (product.throwsTowelsStyles?.includes('XLarge') || 
                product.throwsTowelsStylePrices?.some(style => style.size === 'XLarge'));
      });
  ...
  const getMinPrices = (product: Product) => {
    ...
    for (const style of product.throwsTowelsStylePrices) {
  ...
  useEffect(() => {
    setShowCount(30);
  }, [selectedStyle, selectedColor, priceRange]);
      ...
      try {
        ...
        const validItems = Array.isArray(existingItems) 
          ? existingItems.filter(item => 
              ...
              !item.id.startsWith('xlarge_')
            )
          : [];
        
        const newItems = products
          .filter((p) => newWishlist.includes(`xlarge_${p._id}`))
          .map((item) => ({
            id: `xlarge_${item._id}`,
            src: item.images?.[0] || '',
    ...
    <>
      ...
      <main>
        ...
        <div style={{
          ...
          <Image
            src="/xlarge-throws-towels.jpg"
            alt="XLarge Throws & Towels"
            fill
            style={{
              objectFit: 'cover',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Discover our collection of extra large throws and towels
              </p>
            </div>
          </div>
        </div>
                ...
                <div style={{
                  ...
                  <button
                    ...
                    style={{
                      ...
                      transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                      transform: wishlist.includes(`xlarge_${product._id}`) ? 'scale(1.1)' : 'scale(1)',
                      backdropFilter: 'blur(4px)'
                    }}
                  >
                    <svg
                      ...
                      height="24"
                      viewBox="0 0 24 24"
                      fill={wishlist.includes(`xlarge_${product._id}`) ? '#e53935' : 'none'}
                      stroke="#e53935"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
