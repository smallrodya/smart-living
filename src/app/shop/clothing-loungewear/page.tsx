...
export default function ClothingLoungewearPage() {
    ...
    try {
      ...
      const filteredProducts = data.products.filter((product: Product) => {
        ...
        return product.category === 'CLOTHING' && 
               product.clothingStyles?.includes('Loungewear');
      });
  ...
  const toggleWishlist = (id: string) => {
    setWishlist(prev => {
      const prefixedId = `loungewear_${id}`;
      ...
      try {
        ...
        const validItems = Array.isArray(existingItems) 
          ? existingItems.filter(item => 
              ...
              !item.id.startsWith('loungewear_')
            )
          : [];
        
        const newItems = products
          .filter((p) => newWishlist.includes(`loungewear_${p._id}`))
          .map((item) => ({
            id: `loungewear_${item._id}`,
            src: item.images?.[0] || '',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Discover our collection of comfortable loungewear
              </p>
            </div>
          </div>
        </div>
                  ...
                  >
                    <button
                      ...
                      style={{
                        ...
                        transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                        transform: wishlist.includes(`loungewear_${product._id}`) ? 'scale(1.1)' : 'scale(1)',
                        backdropFilter: 'blur(4px)'
                      }}
                    >
                      <svg
                        ...
                        height="20"
                        viewBox="0 0 24 24"
                        fill={wishlist.includes(`loungewear_${product._id}`) ? '#e53935' : 'none'}
                        stroke="#e53935"
                        strokeWidth="2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
