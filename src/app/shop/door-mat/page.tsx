...
export default function DoorMatPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const doorMats = data.products.filter(
        (product: Product) => 
          (product.category === 'RUGS & MATS' && 
          product.subcategory === 'Door Mat') ||
          (product.additionalCategories && 
           product.additionalCategories.some(
             (ac: { category: string; subcategory: string }) => 
               ac.category === 'RUGS & MATS' && 
               ac.subcategory === 'Door Mat'
           ))
      );
      console.log('Filtered door mats:', doorMats); // Для отладки
      setProducts(doorMats);
    } catch (error) {
      ...
      try {
        ...
        const validItems = Array.isArray(existingItems) 
          ? existingItems.filter(item => 
              ...
              !item.id.startsWith('door_')
            )
          : [];
        
        const newItems = products
          .filter((p) => newWishlist.includes(`door_${p._id}`))
          .map((item) => ({
            id: `door_${item._id}`,
            src: item.images?.[0] || '',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Welcome guests with style using our collection of durable and attractive door mats, perfect for any entrance
              </p>
            </div>
          </div>
        </div>
                ...
                <div style={{
                  ...
                  <button
                    ...
                    style={{
                      ...
                      transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                      transform: wishlist.includes(`door_${product._id}`) ? 'scale(1.1)' : 'scale(1)',
                      backdropFilter: 'blur(4px)'
                    }}
                  >
                    <svg
                      ...
                      height="24"
                      viewBox="0 0 24 24"
                      fill={wishlist.includes(`door_${product._id}`) ? '#e53935' : 'none'}
                      stroke="#e53935"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
